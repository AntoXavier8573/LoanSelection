...
const Dropdown = (props) => {
  ...
  const toggleItem = (item) => {
    ...
    selectedItems_ = selectedItems_.filter((num) => Number(num));
  ...
  const renderItem = ({ item, index }) => {
    return (
      ...
      >
        ...
        >
          {item.TypeDesc}
          {selectedItems.includes(Number(item["TypeOption"]?.toString().trim())) && (
            <Icon
              name="check"
              size={12}
              color="#6c757d"
  ...
  return (
    ...
    >
      ...
      <Icon style={styles.icon} name={`chevron-${visible ? "up" : "down"}`} size={11} color="#6c757d" />
    </TouchableOpacity>
  );
};
